...
build:all
	...
	rm erikOS/boot/grub/ -r -f
	mkdir erikOS/boot/grub/
	echo set default=0 >> erikOS/boot/grub/grub.cfg
	echo set timeout=0 >> erikOS/boot/grub/grub.cfg
	echo menuentry "erikOS" { >> erikOS/boot/grub/grub.cfg
	echo         set root='(hd96)' >> erikOS/boot/grub/grub.cfg
	echo         multiboot /boot/kernel.bin >> erikOS/boot/grub/grub.cfg
	echo } >> erikOS/boot/grub/grub.cfg

	grub-mkrescue -o iknow.iso erikOS/
